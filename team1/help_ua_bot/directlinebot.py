...
class DirectLineAPI(object):
    ...
    def send_message(self, text):
        ...
        url = "/".join(
            [self._base_url, "conversations", self._conversationid, "activities"]
        )
        jsonpayload = {
    ...
    def get_message(self):
        ...
        botresponse = requests.get(
            url, headers=self._headers, json={"conversationId": self._conversationid}
        )
        if botresponse.status_code == 200:
            jsonresponse = botresponse.json()
